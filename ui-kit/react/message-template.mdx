...
The `replyView` method of MessageTemplate allows you to add a reply view to your message bubbles. In the example below, we will add a custom reply view to every text message in the MessageList.
...
const CustomMessageTemplate = () => {
  ...
  useEffect(() => {
    ...
    const template = definedTemplates.map((templates) => {
      if (
        ...
        templates.replyView = (message: CometChat.BaseMessage) =>
          getReplyView(message);
        return templates;
      }
      return templates;
    });
  ...
  const getReplyView = (message: CometChat.BaseMessage) => {
    if (message instanceof CometChat.TextMessage) {
      const timestamp = message.getSentAt();
      const formattedTime = formatTime(timestamp);
      return <div className="reply-view">{formattedTime}</div>;
    } else {
      return null;
    }
  };
...
const CustomMessageTemplate = () => {
    ...
    useEffect(() => {
        ...
        const template = definedTemplates.map((templates) => {
            if (templates.type === CometChatUIKitConstants.MessageTypes.text &&
                templates.category === CometChatUIKitConstants.MessageCategory.message) {
                templates.replyView = (message) => getReplyView(message)
                return templates;
            }
            return templates
        })
