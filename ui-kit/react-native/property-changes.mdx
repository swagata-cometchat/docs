...
| `disableUsersPresence` | `usersStatusVisibility` | boolean | Toggle user status visibility *(inverted logic: `disableUsersPresence=false` → `usersStatusVisibility=true`)*. |
| `hideReceipt` / `disableReceipt` | `receiptsVisibility` | boolean | Controls receipt visibility *(inverted logic: `hideReceipt=false` → `receiptsVisibility=true`)*. |
